...
聚合函数作用于一组数据，并对一组数据返回一个值。只能在使用`GROUP BY`分组之后使用。
...
##### ==问题：**<font color="red">用count(*)，count(1)，count(列名)谁好呢?</font>**==

> ​		其实，对于`MyISAM`引擎的表是没有区别的，因为这种引擎内部有一个计数器在维护着行数。但是对于 `InnoDB`引擎的表用count(*)，count(1)直接读行数，复杂度是O(n)，因为innodb真的要去数一遍。但好于具体的count(列名)，因为count(列名)会去判断列是否为NULL，这会降低统计效率。

* 问题：统计行数时，能不能使用count(列名)替换count(*)？

    ​		不要使用 count(列名)来替代 count(\*) 。 <font style="background-color: #73cf60">count(\*) 是 SQL92 定义的标准**统计行数**的语法，跟数据库无关、跟列值是否为 NULL 和非 NULL 也无关</font>。 说明：**<font color="green">count(*)会统计列值为 NULL 的行，而 count(列名)不会统计列值为 NULL 的行</font>**。
...
3. 添加外部行。如果我们使用的是左连接、右连接或者全连接，就会涉及到外部行，也就是在虚拟 表 vt1-2 的基础上增加外部行，得到虚拟表 vt1-3。
